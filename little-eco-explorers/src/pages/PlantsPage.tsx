...
const PlantsPage = () => {
  ...
  const plants = articles.filter((a) => a.category === "plants");
        ...
        <section className="bg-primary/10 py-16 md:py-24">
          <div className="eco-container text-center">
            ...
            <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold text-foreground mb-4">
              {t("plantsPage.heroTitle")}
            </h1>
            <p className="text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto">
              {t("plantsPage.heroSubtitle")}
            </p>
          </div>
        </section>
        ...
        <section className="eco-section">
          <div className="eco-container">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {plants.map((plant) => (
                <Link key={plant.slug} to={`/articles/${plant.slug}`}>
                  <article className="eco-card overflow-hidden group h-full">
                    ...
                    <div className="p-6">
                      <h3 className="font-display text-xl font-bold text-foreground mb-2 group-hover:text-primary transition-colors">
                        {pick(plant.title, language)}
                      </h3>
                      <p className="text-muted-foreground text-sm mb-4 line-clamp-2">
                        {pick(plant.excerpt, language)}
                      </p>
